...
class AddCargoProductController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('vendor.products.cargo.create', compact('categories', 'measurementUnits', 'countries'));
    }
        ...
        try {
            ...
            if (empty($deliveryOptions)) {
                throw ValidationException::withMessages([
                    'delivery_options' => ['At least one shipping option is required.']
                ]);
            }

            if (count($deliveryOptions) > 4) {
                throw ValidationException::withMessages([
                    'delivery_options' => ['No more than 4 shipping options are allowed.']
                ]);
            }
            ...
            foreach ($deliveryOptions as $index => $option) {
                if (strlen($option['description']) > 255) {
                    throw ValidationException::withMessages([
                        "delivery_options.{$index}.description" => ['Shipping option description cannot exceed 255 characters.']
                    ]);
                }

                if ($option['price'] < 0) {
                    throw ValidationException::withMessages([
                        "delivery_options.{$index}.price" => ['Shipping price cannot be negative.']
                    ]);
                }
            }
